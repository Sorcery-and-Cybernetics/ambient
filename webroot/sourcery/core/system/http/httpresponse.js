...
//todo: add position properties for manual chunked sending
    ...
    _.define.core.object("httpresponse", function () {
        return {
            ...
            , initialize: function (server, request, response) {
                var self = this
            ...
            , flush: function () {
                ...
                if ((this.state == this.states.created) && (!this._mimetype)) {
                    switch (_.vartype(this.sendbuffer)) {
                        case _.vtarray:
    //                        data = JSON.stringify(this.sendbuffer)
                            this.mimetype("json")
    ...
    //                        data = JSON.stringify(this.sendbuffer)
            ...
            , writebuffer: function () {
                ...
                switch (_.vartype(data)) {
                    case _.vtarray:
                    case _.vtobject:
    //                    var stringifytime = _.now()
                        data = JSON.stringify(data)
    //                    _.debug.info(this, "stringify", "", "parsetime: " + (_.now() - stringifytime))
                }

                if (this.compress) {
    //                var ziptime = _.now()
                    data = zlib.gzipSync(data)
    //                _.debug.info(this, "compress", "", "parsetime: " + (_.now() - ziptime))
                }
            ...
            , writeheader: function (httpcode) {
                ...
                //if (this.session && this.session.needsrefresh()) {
                //    this.session.isdirty = false
                //    //_.debug("session: refresh jwt " + JSON.stringify(_.jwt.decode(this.session.tojwt()).payload))
                //    this.sessiontoken(this.session.tojwt())
                //}
...
//                    _.debug.info(this, "performance", [mimetype.contenttype, this.url], "parsetime: " + parsetime)
                    // if (parsetime > this.timeoutwarningduration*100 && this.url =="ocean" ) {
                    //     _.debug.info(this, "performance", [mimetype.contenttype, this.url], "SHIT")
            ...
            , senderror: function (errormessage, errcode, params) {
                //todo: raise error
                //this.raise(errormessage)
            ...
            , senddata: function (data, mimetype) {
                this.state = this.states.sending
            ...
            //todo: postdata buffer should be a separate object
            , getpostdata: function (next) {
                var me = this
                        ...
                        if (mimetype.encoding == "utf-8") {
                            switch (_.left$(data, 1)) {
                                ...
                                case "[":
                                    ...
                                    break
                                default:
                                    //todo: is it always  true that the body is formatted as formdata?
    ...
    //                            , "Transfer-Encoding": "chunked"
            ...
            , handleproxy: function (url) {
                _.state = this.states.proxying
            ...
            //todo: add listeners
            //, ondestroy: _.make.core.signal()
            //, onerror: _.make.core.signal()
            //, ondone: _.make.core.signal()
        }
    })
